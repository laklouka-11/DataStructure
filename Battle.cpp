...
{	
	...
	int ArrivalTime=1;
	Enemy* pE= NULL;
	//Create Random enemies and add them all to inactive queue ==> should be loaded from file
	...
	{			
		...
		pE->SetType( (ENMY_TYPE) (rand() % ENMY_TYPE_CNT));
	...
	pGUI->waitForClick();	//Demo done, wait for a click to exit
}
...
{	
	...
	//2- Add other lists to drawing list
	
	//TO DO
	//In next phases, you should add enemies from different lists to the drawing list
	//For the sake of demo, we only have DemoList, so we will add it to be drawn
